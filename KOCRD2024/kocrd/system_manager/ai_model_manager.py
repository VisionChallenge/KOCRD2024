...
from kocrd. import AITrainingManager
...
class AIModelManager:
    def __init__(self, database_manager, ai_training_manager, config_loader):
        self.config_loader = config_loader
        self.model_loader = ModelLoader(config_loader)
        self.ai_trainer = AITrainer(database_manager, ai_training_manager, config_loader)
        self.tokenizer, self.gpt_model = self.model_loader.load_gpt_model(config_loader.get("file_paths.gpt_model_path", "gpt2"))
        self.text_generator = TextGenerator(self.tokenizer, self.gpt_model)
        self.model = self.model_loader.load_tensorflow_model(config_loader.get("file_paths.ai_model_path"))

    def generate_text(self, command: str) -> str:
        return self.text_generator.generate_text(command)
...
class ModelLoader:
    ...
    def load_tensorflow_model(self, model_path: str) -> tf.keras.Model:
        try:
            return tf.keras.models.load_model(model_path)
        except Exception as e:
            logging.error(f"TensorFlow 모델 로드 실패: {e}")
            return None

    def load_gpt_model(self, gpt_model_path: str):
        try:
            ...
            logging.error(f"GPT 모델 로드 실패: {e}")
            return None, None
class AITrainer:
    def __init__(self, database_manager, ai_training_manager, config_loader):
    ...
    def train_model(self, model, features, label):
        try:
            data = self.database_manager.load_training_data()
            if not data:
            ...
            model_type = self.config_loader.get("ui.settings.ai_model", "classification")
            model = self.config_loader.create_ai_model(model_type)
            if not model:
                raise ValueError(f"AI 모델 생성 실패: {model_type}")
            
            self.ai_training_manager.train_model(model, data, features, label)
        except Exception as e:
            logging.error(f"AI 모델 학습 실패: {e}")
            raise
class TextGenerator:
    ...
    def generate_text(self, command: str) -> str:
        ...
        try:
            ...
            logging.error(f"GPT 텍스트 생성 오류: {e}")
            return "GPT 텍스트 생성 오류"